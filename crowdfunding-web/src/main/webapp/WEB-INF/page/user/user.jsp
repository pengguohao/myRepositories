    ...
    <script type="text/javascript">
         ...
         function useQuery(para){
        	 ...
        	 $($cdBoxes).each(function(){
        		 if(($.inArray($(this).attr("userId"),$checkBoxes))<0){
        		 $checkBoxes.push($(this).attr("userId"));
        		 }
				});
        	 
     		$.ajax({
         	   type: "POST",
         	   url: "${APP_PATH}/user/sel_All_Page",
         	   data: {
         		   "pageNum":para,
         		  	"selAsked":selAsked
         	   },       		   
         	   success: function(result){
       		   		...
       		   		var str_foot="";
           	     	if(result.success){
           	     		var users=result.pageDisp;
           	     		var pageSize=result.pageSize;
           	   			$.each(users,function(i,user){
           	   				str_body+="<tr pageSize='"+pageSize+"' id='user_pageInfo_tr'>"
           	   				str_body+="<td>"+(i+1)+"</td>"
           	   				str_body+="<td><input type='checkbox' userId='"+user.uid+"'></td>"
           	   				str_body+="<td>"+user.username+"</td>"
           	   				str_body+="<td>"+user.password+"</td>"
           	   				str_body+="<td>"+user.pname+"</td>"
           	   				str_body+="<td>"+user.email+"</td>"
           	   				if(user.info==0){
           	   				str_body+="<td>会员&nbsp;&nbsp;<button type='button' class='btn btn-success btn-xs' onclick='overTurn(1,"+user.uid+")'><i class='fa fa-refresh'></i></button></td>"
           	   				}else{
           	   				str_body+="<td>管理&nbsp;&nbsp;<button type='button' class='btn btn-success btn-xs' onclick='overTurn(0,"+user.uid+")'><i class='fa fa-refresh'></i></button></td>"	
           	   				}
           	   				str_body+="<td>"
           	   				str_body+="<button type='button' class='btn btn-success btn-xs' onclick='assignRoleBtn("+user.uid+")'><i class=' glyphicon glyphicon-check'></i></button>"
           	   				str_body+="<button type='button' class='btn btn-primary btn-xs' onclick='updInfoBtn("+user.uid+")'><i class=' glyphicon glyphicon-pencil'></i></button>"
           	   				str_body+="<button type='button' class='btn btn-danger btn-xs' onclick='delOneInfoBtn("+user.uid+")'><i class=' glyphicon glyphicon-remove'></i></button>"
           	   				str_body+="</td>"
           	   				str_body+="</tr>"
             	   		})
         ...
         function assignRoleBtn(uid){
        	 $.ajax({
        	     ...
        	     if(result.ajaxCode){
        	    	layer.close(loadingIndex);
             	    location.href="${APP_PATH}/user/assignRole";
        	     }else{
        	    	layer.close(loadingIndex); 
            	    layer.msg("操作失败，请稍后重试！", {time:1000, icon:5, shift:6}, function(){});	
        	     }
         ...
         function updInfoBtn(uid){
        	 $.ajax({
      	   		 type: "POST",
      	   		 url: "${APP_PATH}/user/user_edit",
        	   	 data: {
        		 "uid":uid
        	   		}, 
        	   	 beforeSend:function(){
        	     ...
        	     if(result.ajaxCode){
        	    	 layer.close(loadingIndex); 
        	    	 location.href="${APP_PATH}/user/edit"
        	     }else{
        	    	 layer.close(loadingIndex); 
        	    	 layer.msg("操作失败，请稍后重试！", {time:1000, icon:5, shift:6}, function(){});
        	     }
         ...
         function overTurn(id,uid){
        	 ...
        	 layer.confirm("是否将该用户身份变更为"+info+"？",  {icon: 3, title:'提示'}, function(cindex){
  			    layer.close(cindex);
  			    
         		 $.ajax({
           	   		 type: "POST",
           	   		 url: "${APP_PATH}/user/upd",
             	   	 data: {
             		 "info":id,
             		 "uid":uid
             	   		}, 
             	     ...
             	     if(result.ajaxCode){
             	    	 layer.close(loadingIndex); 
             	    	 useQuery(1);	 
             	    	 layer.msg("成功变更用户信息！", {time:1000, icon:1, shift:2}, function(){
             	    	 });
             	     }else{
             	    	 layer.close(loadingIndex); 
             	    	 layer.msg("操作失败，请稍后重试！", {time:1000, icon:5, shift:6}, function(){});
